...
def build_messages(history: Sequence, prompt: str) -> List[BaseMessage]:
    ...
    # с блоком описаний документов и перерастает лимит при нескольких больших
    # вложениях. отрицательный бюджет пропускал бы каждую реплику истории
    # через сравнение cost > budget, и запись в лог ниже сообщала бы об обрезке
    # истории вместо действительной причины
    budget = max(0, MAX_CONTEXT_CHARS - len(prompt))

    # запрос длиннее лимита уходит на сервер как есть: обрезка текста письма
    # оставила бы вопрос без части условий. запись в лог называет причину
    # пустого контекста, которую иначе пришлось бы искать по ответам модели
    if len(prompt) >= MAX_CONTEXT_CHARS:
        log.warning(
            "запрос длиной %d символов достиг лимита %d: история сессии в него не вошла",
