...
def _reply(
    ...
    # sender_name, quoted_body, sent_date и quoted_html дают reply_builder
    # цитату входящего письма — без неё ответ доходит без блока «Reply»,
    # по которому Outlook показывает, на какое именно письмо отвечает модель.
    #
    ...
    #
    # quoted_html несёт разметку письма как есть: цитата в html-части
    # воспроизводит письмо пользователя без потери начертания, таблиц
    # и картинок. в запрос к модели и в таблицу messages разметка не идёт —
    # там остаётся incoming.body, очищенный от цитат
    return transport.send_reply(
        ...
        quoted_html=incoming.body_html,
    )
