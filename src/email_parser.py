...
def parse_email(msg: Message) -> IncomingEmail:
    ...
    #
    # у пересланного письма пустой результат отсечения означает пересылку
    # без единого слова от себя: восстанавливать здесь нечего, всё тело
    # состоит из чужого треда без указания авторства реплик. такое тело
    # ушло бы в модель и в таблицу messages репликой пересылающего, поэтому
    # body остаётся пустым, и pipeline просит сформулировать вопрос текстом
    if len(body) < MIN_BODY_CHARS and raw_body.strip() and not forwarded:
