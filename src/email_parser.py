...
def parse_email(msg: Message) -> IncomingEmail:
    ...
    #
    # у пересланного письма отсечение промахивается систематически: маркер
    # REPLY_MARKER внутри пересылаемой переписки с моделью читается как начало
    # НАШЕГО письма, и strip_quoted обрезает по нему всё, включая новый текст
    # пересылающего. частый случай — второй пользователь получил переписку
    # с моделью пересылкой и пересылает её в ящик модели повторно: для его
    # сессии эта переписка не дублирует историю (лежит в сессии первого
    # пользователя), поэтому восстанавливается без strip_own_replies —
    # реплики модели внутри остаются текстом, на который модель может
    # сослаться. strip_own_replies применяется только вне пересылки, где
    # риск дублирования реален: письмо своего же треда, где клиент подклеил
    # прежний наш ответ снизу
    if len(body) < MIN_BODY_CHARS and raw_body.strip():
        ...
        cleaned = raw_body if forwarded else strip_own_replies(raw_body)
        body = strip_header_blocks(cleaned) or raw_body.strip()
