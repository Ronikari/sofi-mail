...
Письмо уходит двумя частями (`text/plain` и `text/html`), ответ набран Liberation Serif 12pt.
Под подписью стоит цитата письма пользователя с шапкой `От:/Отправлено:/Кому:/Тема:`: в html-части
его разметка воспроизводится без изменений, в цитату идёт тело письма целиком — в треде Outlook
видна вся переписка сессии. В запрос к модели цитата не попадает, там остаётся только новый текст.
...
|---|---|
| `Dockerfile` | две стадии: зависимости, боевой образ |
...
<!-- стадия test снята из сборки вместе с `requirements-dev.txt` и хуком
     `.githooks/pre-push`; строка таблицы сохранена на случай её возврата:
