...
# Пусто = ходить напрямую в pypi.org и deb.debian.org (сборка на машине
# с доступом в интернет). В контуре компании сюда подставляются адреса Nexus.
ARG PIP_INDEX_URL=""
...
# --- Тесты ------------------------------------------------------------------
# Отдельная стадия, а не шаг боевого образа: тесты гоняются по требованию
# (`docker build --target test .`) и в него не попадают. Проверяют они ровно тот
# набор зависимостей, который уедет в боевой слой, — сетевые вызовы подменены
# заглушками (tests/conftest.py), Exchange и модель для прогона не нужны.
...
#
# Это пользователь ОС внутри контейнера, к домену и к Exchange отношения не
# имеющий: на EWS демон входит под доменной учёткой из MAIL_LOGIN, а здесь речь
# только о том, кому принадлежит процесс и каталог с базой. Совпадение имени
# с адресом ящика случайное
